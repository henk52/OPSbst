...
# verify that filename exists.
  ...
  if ( -f $szFileName ) {
   ...
   confess("!!! File does not exist: $szFileName");
  }
  if ( defined($tree) ) {
   $root = $tree->getDocumentElement;
  } else {
...
sub GetDataArrayByTagName {
  if ( $#_ != 1 ) {
    die "GetDataArrayByTagName(): wrong parameter number params($#_), need at least xmlNode and TagName ", __FILE__, " line ", __LINE__, "\n";
  }
